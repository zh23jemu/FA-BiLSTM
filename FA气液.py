...
def load_fluid_training_sequences(current_dir, features, window_size):
    training_sources = []

    base_df = pd.read_excel(os.path.join(current_dir, '6-3训练数据_副本3.xlsx'))
    training_sources.append(('原始流体训练井段', base_df))

    extra_file = os.path.join(current_dir, '扩充数据.xlsx')
    if os.path.exists(extra_file):
        extra_df = pd.read_excel(extra_file, sheet_name='扩充数据2')
        extra_df = extra_df.copy()
        if 'SGFC' in extra_df.columns and 'Sigma' not in extra_df.columns:
            extra_df['Sigma'] = extra_df['SGFC']
        training_sources.append(('扩充数据2', extra_df))
    ...
    for source_name, df_source in training_sources:
        if not set(features + ['label2']).issubset(df_source.columns):
            continue
        ...
        _, valid_seq_indices, y_train_seq = create_sequences(X_source, df_source['label2'], window_size)
...
X_train_seq, y_train_seq, source_stats = load_fluid_training_sequences(current_dir, features, WINDOW_SIZE)
